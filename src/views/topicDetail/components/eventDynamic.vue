<template>
  <div v-if='dynamicList.length !== 0 ' style='width:100%'>
    <div v-for='(item,index) in dynamicList' :key='index' class='condition-dynamic'>
      <!-- 用户头像 -->
      <div class='user'>
        <!-- 小log -->
        <div v-show="item.smallImg !== ''" class='icon'>
          <img :src="item.smallImg + '?param=18y18'" alt=''>
        </div>
        <img v-imgLazy :data-src="item.userimg + '?param=50y50'" alt='' src=''>
      </div>
      <!-- 用户分享内容 -->
      ...
      <!-- 歌曲或mv或歌单区域 -->
      <div v-if="(item.ids !== false && item.ids.type !== 'mv') " class='songContent' @click='shareContentClick(0,item.ids.type,item.ids.id)'>
        <div class='content_img' @click.stop='shareContentClick(1,item.ids.type,item.ids.id)'>
          <img v-imgLazy :data-src="item.contentImg + '?param=50y50'" alt='' src=''>
          <div class='stop_music'>
            <img alt='' src='@/assets/img/stopMusic.svg'>
          </div>
        </div>
      ...
      <!-- 分享图片 -->
      <div v-if='item.pics.length !== 0' class='share_img'>
        <ul v-if='item.pics.length > 1'>
          <li v-for='(items, index) in item.pics' :key='index'>
            <img v-imgLazy :data-src="items.pcSquareUrl  + '?param=302y302'" alt='' src='' @click.stop='showPicsList(index,item.pics)'>
          </li>
        </ul>
        <img v-else v-imgLazy :data-src="item.pics[0].pcSquareUrl + '?param=566y566'" alt='' src='' @click.stop='showPicsList(0,item.pics)'>
      </div>
    </div>
  </div>
</template>
...
import { _getMusicListDetail, _getSongsDetail, AllSongDetail, SongDetail } from '@/network/detail';
