<template>
  <el-container class="p-30">
    ...
    <el-main>
      ...
      <meSongList v-if="userSongList.length !== 0" :songList="userSongList" :isTable="tableImageIcon" />
    </el-main>
  </el-container>
</template>
<script>
import { _getSongList, _getUserInfo } from '@/network/user';
import meSongList from './meSongList';
export default {
  ...
  data() {
    return {
      createSongList: [],
      collectSongList: [],
      tableImageList: [{ img: require('@/assets/img/table01.svg') }, { img: require('@/assets/img/table02.svg') }, { img: require('@/assets/img/table03.svg') }],
      tableImageIndex: -1,
      tableImageIcon: 0,
      userName: '',
      userGender: '',
