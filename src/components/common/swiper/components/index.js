export function _Swiper(oSwiper) {
  ...
  let swiper = oSwiper;
  // 右按钮
  let pre = swiper.getElementsByClassName('pre')[0];
  // 左按钮
  let next = swiper.getElementsByClassName('next')[0];
  // 右边背景
  let left = swiper.getElementsByClassName('left')[0];
  // 左边背景
  let right = swiper.getElementsByClassName('right')[0];
  // li标签
  let li = swiper.querySelectorAll('li');
  // span小线条
  let span = swiper.querySelectorAll('span');
