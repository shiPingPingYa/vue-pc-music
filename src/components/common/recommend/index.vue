<template>
  <div class="recommond">
    ...
    <!-- 评论区域 -->
    <div class="recommend-content-container" v-if="recommends.length !== 0">
      <div class="hot_comments" v-if="hotComments.length !== 0">
        <p>热门评论</p>
        <HotComments :hotComments="hotComments" />
      </div>
      <p>精彩评论</p>
      <div class="recommend-content" v-for="(item, index) in recommends" :key="index">
        <div class="icon">
    ...
    <div class="recommend-content-container" v-else-if="recommends.length == 0">
      <p>精彩评论</p>
...
export default {
  ...
  methods: {
    ...
    async setCommentsLikedCount(commentId) {
      ...
      const {
        data: { code }
      } = await _setCommentsLikedCount(params);
